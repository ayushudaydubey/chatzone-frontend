...
const UserList = ({
  users = [
    ...
    { username: 'elva_ai', isOnline: true },
    { username: 'jane_smith', isOnline: false },
    { username: 'mike_wilson', isOnline: true },
    { username: 'sarah_connor', isOnline: true }
  ],
  toUser = '',
  setToUser = (user) => console.log('Selected user:', user),
  ...
  getUnreadCount = (user) => {
    // Mock unread counts
    const counts = { 'john_doe': 3, 'jane_smith': 1, 'mike_wilson': 0, 'sarah_connor': 5 };
    return counts[user] || 0;
  },
  getLastMessage = (user) => {
    // Mock last messages
    const messages = {
      'john_doe': { message: 'Hey, how are you doing?', timestamp: Date.now() - 300000 },
      'elva_ai': { message: 'I can help you with that!', timestamp: Date.now() - 60000 },
      'jane_smith': { message: 'Thanks for the help!', timestamp: Date.now() - 3600000 },
      'mike_wilson': { message: 'See you tomorrow', timestamp: Date.now() - 86400000 },
      'sarah_connor': { message: 'Great work on the project', timestamp: Date.now() - 1800000 }
    };
  ...
  const isFileUrl = (message) => {
    ...
    return message.includes('imagekit.io') || 
           message.includes('cloudinary.com') ||
           message.includes('amazonaws.com') ||
           message.match(/\.(jpg|jpeg|png|gif|webp|svg|bmp|ico|mp4|avi|mov|pdf|doc|docx|txt)$/i);
  };
  ...
  const isFileMessage = (message) => {
    ...
    return message.messageType === 'file' || 
           ...
           (!message.fileInfo && isFileUrl(message.message));
  };
          ...
          <div className="flex items-center gap-2">
            ...
            {totalUnread > 0 && (
              <span className="animate-pulse shadow-sm shadow-pink-600 bg-gradient-to-b from-red-600/70 to-pink-700/70 absolute top-5 right-10 text-white text-xs px-2 py-2 rounded tracking-wide font-thin">
                <span>{totalUnread} New Messages</span>
              </span>
            )}
          </div>
          <button
            ...
            <div className="min-h-full">
              ...
              {allUsers.length > 0 ? (
                allUsers.map((user, index) => {
                  ...
                  const { username: userName, isOnline = true } = userObj;
                  ...
                  const isSelected = toUser === userName;
                  ...
                  if (lastMessage) {
                    if (isFileMessage(lastMessage)) {
                      lastMessageText = lastMessage.fileInfo?.fileName || '📎 File';
                    } else {
                      lastMessageText = truncateMessage(lastMessage.message || '');
                    }
                    lastMessageTime = formatLastMessageTime(lastMessage.timestamp);
                  }
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            <p className={`font-medium capitalize truncate text-sm sm:text-base ${
                              isAI ? 'text-blue-200' : 'text-white'
                            }`}>
                          ...
                          {lastMessageTime && (
                            <span className={`text-xs flex-shrink-0 ml-2 ${
                              isAI ? 'text-blue-200' : 'text-gray-400'
                            }`}>
                              {lastMessageTime}
                            </span>
                          )}
                        ...
                        {lastMessage ? (
                          <div className="flex items-center gap-1">
                            {isFileMessage(lastMessage) ? (
                              <>
                                <span className="text-xs">📎</span>
                                <p className={`text-xs truncate ${
                                  isAI ? 'text-teal-200' : 'text-gray-400'
                                }`}>
                                  Sent a file
                                </p>
                              </>
